...
def proc(image):
    cv2.imwrite("../proc_imgs/2_1_org.jpg", image)
    ...
    top_x1,top_y1=int(top_x1),int(top_y1)
    top_x2,top_y2 = top_half[1]
    top_x2,top_y2=int(top_x2),int(top_y2)
    bottom_x1,bottom_y1 = bottom_half[0]
    bottom_x1,bottom_y1 =int(bottom_x1),int(bottom_y1)
    bottom_x2,bottom_y2 = bottom_half[1]
    bottom_x2,bottom_y2=int(bottom_x2),int(bottom_y2)
...
if __name__ == '__main__':
    ...
    image = cv2.imread("../imgs/4_down.jpg")
    image=proc(image)
    cv2.imwrite("./proc_imgs/upside_2_final.jpg", image)